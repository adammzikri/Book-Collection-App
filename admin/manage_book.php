		...
		<div class="col-lg-12">
			...
			<div class="row">
				<div class="col-md-6 border-right">
					...
					<div class="form-group">
						<label class="label control-label">Year</label>
						<input type="text" class="form-control form-control-sm w-100" name="year" required="" value="<?php echo isset($year) ? $year : '' ?>">
					</div>
					<div class="form-group">
					...
					<div class="form-group">
						...
						<select name="author_ids[]" id="" class="custom-select custom-select-sm select2" required >
							<option value=""></option>
							<?php
							$author = $conn->query("SELECT * FROM authors order by name asc");
							while($row= $author->fetch_assoc()):
							?>
							<option value="<?php echo $row['id'] ?>" <?php echo isset($author_ids) && !empty($author_ids) &&  in_array($row['id'],explode(',',$author_ids)) ? 'selected' : '' ?>><?php echo ucwords($row['name']) ?></option>
						<?php endwhile; ?>
						</select>
					</div>
				</div>
